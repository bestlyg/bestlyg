...
import {
  calcHashByWebWorker,
  calcHashSampleByWebWorker,
  calcHashSample,
  calcHashByIdle,
  calcHashSync,
} from './util/calcFileHash';
...
const upload = async () => {
  ...
  const { uploaded, uploadedList } = await verify(file.name, obj.hashValue);
...
const handleResume = async () => {
  status.value = Status.uploading;
  const { uploadedList } = await verify(fileList.value!.name, fileHash.value);
  // 上传切片
...
const handleUpload1 = async () => {
  ...
  while (cur < fileSize) {
    ...
    // 新的切片大小等比变化
    console.log(
      `切片${count}大小是${format(offset)},耗时${time}秒，是10秒的${rate}倍，修正大小为${format(
        offset / rate
      )}`
    );
...
const downloadFile = async () => {
  // StreamSaver
  const file = 'b754ed0a920ec789b98bf00a3c1c0c2a.txt';
  const url = `http://localhost:4001/download?filename=${file}`;
