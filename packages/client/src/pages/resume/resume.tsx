import { ResumeGenerator } from "md2resume";
...
export function Resume() {
  ...
  const [resumePageType, setResumePageType] = useState(
    ResumePageType.MultiPage
  );
  const [resumeSource, onResumeSourceChange] = useControllableValue(
    {
      resumeSource: data ?? "",
    },
    {
  ...
  useEffect(() => {
    async function run() {
      const resumeGenerator = (resumeGeneratorRef.current =
        new ResumeGenerator());
          ...
          <div className={clsx(resumeStyles["resume-toolkits"])}>
            <div className={clsx(resumeStyles["resume-toolkits—inner"])}>
              <Space>
                ...
                {/* <Button onClick={editorVisbileActions.setRight}>
                  打开编辑器
                </Button> */}
              </Space>
            </div>
