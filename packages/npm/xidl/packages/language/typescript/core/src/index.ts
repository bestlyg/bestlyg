...
export class XIdl extends XIdlCore {
    ...
    async genField(field: pb.Field, requiredList: string[]): Promise<string> {
        let str = this.config.indent;
        str += field.name;
        if (field.optional && !requiredList.includes(field.name)) str += '?';
        str += ': ';
        if (field.map) {
    ...
    async extractInfoFromType(field: pb.Type) {
        ...
        if (field.options?.[EXTENDS_KEY]) {
            const parent = field.options[EXTENDS_KEY];
            data.extends = `extends ${parent}`;
        }
        const REQUIRED_KEY = '(required)';
        if (field.options?.[REQUIRED_KEY]) {
            data.required.push(...field.options[REQUIRED_KEY].split(','));
        }
        return data;
    }
}
