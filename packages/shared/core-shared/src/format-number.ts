...
/** 数字格式化正则 */
export const FormatNumberReg =
    /^(?<prefix>[^,.\d]*)(?<integer>,)?(?<decimals>\.\df)?(?<suffix>[^,.\d]*)$/;
...
function parseTemplate(template: string): FormatOptions {
    ...
    if (match && match.groups) {
        ...
        options.decimals =
            match.groups.decimals !== undefined ? parseInt(match.groups.decimals[1]) : undefined;
...
export function formatNumber(
    ...
    const { prefix, decimals, suffix, useThousandSeparator, isPercentage, isPercentagePoint } =
...
export const formatThousands = (value: unknown) => {
    ...
    if (typeof value === 'string') {
        ...
        const hasLeadingZero =
            trimmed.startsWith('0') && !trimmed.startsWith('0.') && trimmed.length > 1;
